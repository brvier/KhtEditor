...
from PySide.QtCore import QSettings, Qt, QObject, Slot, Signal, Property
...
class QmlSettings(QObject):

    themeChanged = Signal()
    fontFamillyChanged = Signal()
    fontSizeChanged = Signal()
    
    def __init__(self):
        self.settings = QSettings()
        self._theme = ''
        self._fontFamilly = 'Monospace'
        self._fontSize = 11
    ...
    @Slot(unicode)
    def setFontFamilly(self,fontFamilly):
        self._fontFamilly = fontFamilly
        self.settings.setValue('fontName',fontFamilly)
        self.settings.sync()                
    @Slot(result=unicode)
    def getFontFamilly(self):        
        if self.settings.contains('fontName'):
            self._fontFamilly = self.settings.value('fontName')
        return self._fontFamilly

    @Slot(int)
    def setFontSize(self,fontSize):
        self._fontSize = fontSize
        self.settings.setValue('fontSize',fontSize)
        self.settings.sync()                
    @Slot(result=int)
    def getFontSize(self):        
        if self.settings.contains('fontSize'):
            self._fontSize = self.settings.value('fontSize')
        return self._fontSize
                
    theme = Property(unicode,getTheme, setTheme, notify=themeChanged)
    fontFamilly = Property(unicode,getFontFamilly, setFontFamilly, notify=fontFamillyChanged)    
